...
class CurlClient(object):
    ...
    def postXmlSSL(self, xml, url, second=30, cert=True, post=True):
        ...
        if cert:
            ...
            self.curl.setopt(pycurl.SSLKEY, current_app.config['WXMCH_SSLKEY_PATH'])
            self.curl.setopt(pycurl.SSLCERTTYPE, "PEM")
            self.curl.setopt(pycurl.SSLCERT, current_app.config['WXMCH_SSLCERT_PATH'])
        #post提交方式
        if post:
...
class HttpClient(Singleton):
    @classmethod
    def configure(cls):
        if pycurl is not None and current_app.config['HTTP_CLIENT'] != "URLLIB":
            return CurlClient
        else:
            return UrllibClient
...
class Common_util_pub(object):
    ...
    def getSign(self, obj):
        ...
        #签名步骤二：在string后加入KEY
        String = "{0}&key={1}".format(String,current_app.config['WXMCH_KEY'])
        #签名步骤三：MD5加密
...
class JsApi_pub(Common_util_pub):
    ...
    def createOauthUrlForCode(self, redirectUrl, scope="snsapi_base"):
        """生成可以获得code的url"""
        urlObj = {}
    ...
    def createOauthUrlForOpenid(self):
        ...
        urlObj = {}
        urlObj["appid"] = current_app.config['WXAPP_ID']
        urlObj["secret"] = current_app.config['WXAPP_SECRET']
    ...
    def getOpenid(self):
        ...
        print 'getopenid', data
        self.result = json.loads(data)
        self.openid = self.result['openid']
        return self.openid
    ...
    def  getParameters(self):
        """设置jsapi的参数"""
        jsApiObj = {}
        jsApiObj["appId"] = current_app.config['WXAPP_ID']
...
class Wxpay_client_pub(Common_util_pub):
    ...
    def createXml(self):
        ...
        self.parameters["appid"] = current_app.config['WXAPP_ID']   #公众账号ID
        self.parameters["mch_id"] = current_app.config['WXMCH_ID']   #商户号
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def getPrepayId(self):
        ...
        self.postXml()
        self.result = self.xmlToArray(self.response)
        print "getPrepayId: %s" % self.result
...
class SendRedPack_pub(Wxpay_client_pub):
    """发送红包接口"""
    def __init__(self, timeout=30):
        self.url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack"
        self.curl_timeout = timeout
        super(SendRedPack_pub, self).__init__()

    def createXml(self):
        """生成接口参数xml"""
        if any(self.parameters[key] is None for key in ("mch_billno", "wxappid", "nick_name", "send_name", "re_openid", "total_amount", "min_value", "max_value", "total_num", "wishing", "client_ip", "act_name", "remark")):
            raise ValueError("missing parameter")
        self.parameters["mch_id"] = current_app.config['WXMCH_ID']  #商户号
        self.parameters["nonce_str"] = self.createNoncestr()  #随机字符串
        self.parameters["sign"] = self.getSign(self.parameters)  #签名
...
class MchPromotion_pub(Wxpay_client_pub):
    '''企业向微信用户个人付款'''
    def __init__(self, timeout=30):
        self.url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers" 
        self.curl_timeout = timeout
        super(MchPromotion_pub, self).__init__() 

    def createXml(self):
        """生成接口参数xml"""
        if any(self.parameters[key] is None for key in ("partner_trade_no", "openid", "check_name", "amount", "desc", "spbill_create_ip")): 
            raise ValueError("missing parameter")

        self.parameters["mchid"] = current_app.config['WXMCH_ID']  #商户号
        self.parameters["nonce_str"] = self.createNoncestr()  #随机字符串
        self.parameters["mch_appid"] = current_app.config['WXAPP_ID']   
        self.parameters["sign"] = self.getSign(self.parameters)  #签名
        return  self.arrayToXml(self.parameters)
...
class NativeCall_pub(Wxpay_server_pub):
    ...
    def createXml(self):
        ...
        if self.returnParameters["return_code"] == self.SUCCESS:
            self.returnParameters["appid"] = current_app.config['WXAPP_ID']  #公众账号ID
            self.returnParameters["mch_id"] = current_app.config['WXMCH_ID']  #商户号
