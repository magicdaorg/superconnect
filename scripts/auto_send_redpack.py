...
def sendRedpack(openid, mch_billno, number):
    ...
    handler.setParameter('client_ip', '139.129.5.136') 
...
def sendMchPromotion(openid, partner_trade_no, number):
    ...
    handler.setParameter('spbill_create_ip', '139.129.5.136') 
...
def autoSendRedpack(wxmch_id=config.SuperConnect.WXMCH_ID, number_per_redpack=200):
    with appinstance.app_context():
        ...
        while True:
            if break_flag:
                break 
            waiting_to_sends_logs = models.ExtractionCommissionLog.objects(status=1)[0: 15] #通过申请 
            if waiting_to_sends_logs.count() <= 0:
                print "Sleeping %s seconds...." % 5 
                time.sleep(5) 
            for i in waiting_to_sends_logs:
                print i 
                is_success = True 
